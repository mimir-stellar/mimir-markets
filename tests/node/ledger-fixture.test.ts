...
test("saved funded ledger replays to the reviewed read-index artifact", async () => {
  ...
  assert.equal(
    artifact.rows[0].creator,
    "GAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAWHF",
  );
});
test("replay is deterministic across fixture loads", async () => {
  assert.deepEqual(
    replayLedgerFixture(await loadLedgerFixture(fixturePath)),
    replayLedgerFixture(await loadLedgerFixture(fixturePath)),
  );
});
test("tampered money data fails the reviewed fingerprint", async () => {
  const fixture = await loadLedgerFixture(fixturePath);
  fixture.events[1].stakeUnits = 99n;
  assert.throws(
    () => replayLedgerFixture(fixture),
    /fingerprint differs from the reviewed fixture/,
  );
});
