...
lib/ops/release-readiness.ts	Machine-readable release readiness checklist (gates, failure/rollback/artifact/secret/env behavior, evidence evaluation)
docs/RELEASE_READINESS.md	The checklist, its per-gate runbook, and the evidence format
...
scripts/verify-release-readiness.ts	CLI: release readiness checklist (npm run verify:release-readiness)
...
the source of truth — re-warm from chain (npm run warm:vs-index / npm run sync). Restoring never writes to the chain.
