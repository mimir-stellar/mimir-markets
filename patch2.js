...
let enJson = fs.readFileSync('messages/en.json', 'utf8');

enJson = enJson.replace(
  '"messagesHub": {',
  '"agentDirectory": {\n    "emptyStateTitle": "No agents found",\n    "emptyStateDescription": "No agents are configured in this deploy. Start by creating the first agent.",\n    "errorTitle": "Could not load agents",\n    "errorDescription": "There was a problem reading the directory. This might be a temporary dependency failure or a network issue.",\n    "createAction": "Create an agent"\n  },\n  "messagesHub": {'
);

fs.writeFileSync('messages/en.json', enJson);

let emptyState = `import { useTranslations } from "next-intl";
...
fs.writeFileSync('components/agents/AgentDirectoryEmptyState.tsx', emptyState);
